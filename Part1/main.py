...
if __name__ == '__main__':
    ...
    print("don't run this line again")
    start = time.time()
    ...
    def testLines():
        for point in edgePixelLocations:
            #Iterate over 90 line angles from 0 to pi
    ...
    def testLinesParallel(pointLocations):
        for point in pointLocations: # used to be edgePixelLocations
            #Iterate over 90 line angles from 0 to pi
            for i in range(accResolutionY): # sizeY
                theta = i * math.pi/accResolutionY

                #Iterate over 90 line distances from 0 to imgDiagSize
                for j in range(accResolutionX): # sizeX
                    rho = (j-accResolutionX//2) * 2*imgDiagSize/accResolutionX
    ...
    def runInParallel(threads):
        proc = []
        pixelLength = len(edgePixelLocations)
        for t in range(threads):
            #p = mp.Process( testLinesParallel( (accResolutionY/threads) * t, accResolutionX) )
            print( (pixelLength/threads)*t)
            print((pixelLength/threads *(t+1)) -1)
            p = mp.Process( testLinesParallel(edgePixelLocations[ (int) (pixelLength/threads)*t : (int) (pixelLength/threads *(t+1)) -1]) )
            p.start()
            proc.append(p)
        for p in proc:
            p.join()
    ...
    # how many votes count to check a line?
